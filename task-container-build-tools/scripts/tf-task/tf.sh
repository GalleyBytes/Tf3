...
function fixssh {
  mkdir -p "$I3_ROOT_PATH"/.ssh/
  if stat "$I3_SSH"/* >/dev/null 2>/dev/null; then
  cp -Lr "$I3_SSH"/* "$I3_ROOT_PATH"/.ssh/
  chmod -R 0600 "$I3_ROOT_PATH"/.ssh/*
  fi
}
...
if [[ "$I3_TASK" == "plan" ]] && [[ "$I3_REQUIRE_APPROVAL" == "true" ]]; then
  printf "Waiting for approval\n"
  until stat "${I3_GENERATION_PATH}/_approved_${POD_UID}" 2>/dev/null >/dev/null ||\
        stat "${I3_GENERATION_PATH}/_canceled_${POD_UID}" 2>/dev/null >/dev/null; do
          sleep 1;
  done
  if stat "${I3_GENERATION_PATH}/_canceled_${POD_UID}" 2>/dev/null >/dev/null; then exit 2;fi
  if stat "${I3_GENERATION_PATH}/_approved_${POD_UID}" 2>/dev/null >/dev/null; then exit 0;fi
fi
